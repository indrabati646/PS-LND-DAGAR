...
# ==============================================================================
# 4. DATA-GENERATING TRUE NON-LINEAR SURFACE
# ==============================================================================
...
run_replicate <- function(rep_id) {
  ...
  # Stage 1: Propensity Score Estimation using separate DAGAR structural loops
  ps_fit <- fit_ps_dagar(X, Z, cluster_id, adj, ordering = ordering)
  e_hat <- ps_fit$e_hat
  
  # Stage 2: BCF Outcome Model Estimation with Censoring Data-Augmentation
  ...
  for(d in 1:4) {
    ...
    # Mathematical Counterfactual Ground Truth
    ...
    # Aggregate draws for subject clusters inside target county
