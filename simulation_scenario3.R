...
b2_func_true <- function(x_vals, z_val, w_val) {
  ...
  main_effect <- 1.0 * z_val + 1.2 * x1 - 1.0 * x2 + 0.3 * (z_val * x1 * exp(x2)) +
...
run_replicate <- function(rep_id) {
  ...
  # --- Setup Grid & Spatial Random Effects ---
  ...
  X1 <- runif(n, 0, 1); X2 <- rbeta(n, 2, 6); X3 <- rbinom(n, 1, 0.8)
  X4 <- rbinom(n, 1, 0.7); X5 <- sample(1:10, n, replace = TRUE)
